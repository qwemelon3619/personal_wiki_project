// services/gallery-service/internal/repository/redis_repo.go
package repository
...
// GetPhotoMetadata retrieves cached photo metadata by ID
func (r *RedisRepoImpl) GetPhotoMetadata(ctx context.Context, photoID string) (*model.Photo, error) {
	...
	if err != nil {
		...
		return nil, nil // Non-fatal for read service
	}
...
// InvalidatePhotoCache removes a photo from cache
func (r *RedisRepoImpl) InvalidatePhotoCache(ctx context.Context, photoID string) error {
	key := "photo:" + photoID
	err := r.client.Del(ctx, key).Err()
	if err != nil && err != redis.Nil {
		log.Printf("[Redis] Failed to invalidate cache for photo %s: %v", photoID, err)
	}
	return err
}
